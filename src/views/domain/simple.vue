  ...
  <div class="simple">
    ...
    <div class="con">
      ...
      <div class="url_con" v-if="tableType">
        ...
        >
          <el-table-column label="URL" prop="url" width="280px"
            ><template slot-scope="scope">
              <span style="color:red" v-if="scope.row.err == false">{{
                scope.row.url
              }}</span>
              <span style="color:green;" v-else>{{ scope.row.url }}</span>
            </template></el-table-column
          >
          <el-table-column prop="url_name" label="视频名称">
            <template slot-scope="scope">
              <span style="color:red" v-if="scope.row.err_url_name == false">{{
                scope.row.url_name
              }}</span>
              <span style="color:green;" v-else>{{ scope.row.url_name }}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="url_type"
            ...
            width="210px"
            ><template slot-scope="scope">
              <span style="color:red" v-if="scope.row.err_url_type == false">{{
                scope.row.url_type
              }}</span>
              <span style="color:green;" v-else>{{ scope.row.url_type }}</span>
            </template></el-table-column
          >
          <el-table-column prop="buser_id" label="渠道ID"></el-table-column>
          <el-table-column prop="label" label="标签">
            <template slot-scope="scope">
              <span v-if="scope.row.err_label == false" style="color:red">{{
                scope.row.label
              }}</span>
              <span v-else style="color:green;">{{ scope.row.label }}</span>
            </template>
          </el-table-column>
          <el-table-column prop="label2" label="视频终端">
            <template slot-scope="scope">
              <span v-if="scope.row.err_label2 == false" style="color:red">{{
                scope.row.label2
              }}</span>
              <span v-else style="color:green;">{{ scope.row.label2 }}</span>
            </template>
          </el-table-column>
          <el-table-column label="上传状态">
            <template slot-scope="scope">
              <span v-if="scope.row.status == '成功'" style="color:green"
                >成功</span
              >
              <span v-else-if="scope.row.status == '待上传'" style="color:grey"
                >待上传</span
              >
        ...
        if (res.err_code == 0) {
          ...
          for (var i = 0; i < res.data.length; i++) {
            ...
            if (this.getBLen(res.data[i].url) > 1024) {
              res.data[i].err = false;
              this.disable = true;
              res.data[i].status += `url长度不能大于1024字符\n`;
            }
            if (resyzm.test(res.data[i].url) === false) {
              res.data[i].err = false;
              this.disable = true;
              res.data[i].status += "url格式错误\n";
            }
            if (res.data[i].url_type >= 3 || res.data[i].url_type < 0) {
              res.data[i].status += "视频类型错误\n";
              res.data[i].err_url_type = false;
            }
            if (this.getBLen(res.data[i].url_name) < 4) {
              res.data[i].status += "视频名称不能小于4字符\n";
              res.data[i].err_url_name = false;
            }
            if (resship.test(res.data[i].url_name) === false) {
              res.data[i].status += "视频名称格式错误\n";
              res.data[i].err_url_name = false;
            }
            if (this.getBLen(res.data[i].url_name) > 50) {
