    ...
    <div class="yure_content">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane label="刷新缓存" name="first">
          ...
          <span style="color: #666666;font-size:14px;">刷新区域：</span>
          <el-cascader
            :options="citylist1"
            ref="cascaderAddr"
        ...
        <el-tab-pane label="操作记录" name="third">
          ...
          >
            <el-table-column prop="url" label="操作内容"></el-table-column>
            ...
            <el-table-column prop="operation_type" label="操作类型"
              ><template slot-scope="scope">
                <span v-if="scope.row.refresh_type == 1">内容预热</span
                ><span v-else>缓存刷新</span></template
              ></el-table-column
            >
            <el-table-column prop="operation_status" label="状态"
              ><template slot-scope="scope">
                <span v-if="scope.row.state == 0">进行中</span>
                <span v-else-if="scope.row.state == 1">完成</span>
                <span v-else-if="scope.row.state == 2">等待</span>
                <span v-else-if="scope.row.state == 3">失败</span>
              </template>
            </el-table-column>
            <el-table-column prop="schedule" label="进度">
              <template slot-scope="scope">
                <div v-if="scope.row.state == 0" style="color:#297AFF">
                  — —
                </div>
                <el-progress
                  v-else
            ...
            if (res.result.cols.length <= 0) {
              this.$alert(
                ...
                "温馨提示",
                {
                  ...
                  center: true,
                  callback: action => {
                    this.$router.push({ path: "/terminal_management" });
                  }
                }
              );
            }
    ...
    getrefresh(datas) {
      ...
      if (datas == 0) {
        ...
        if (this.textarea1 == "") {
          ...
          if (this.removeEmpty(this.textarea1.split("\n")).length > 10) {
            this.$message.error("url最多输入10条");
            return false;
          } else {
            let urlyure = this.removeEmpty(this.textarea1.split("\n"));
            ...
            urlyure.forEach((item, index) => {
              if (resyzm.test(item) === false) {
                this.errarrs = this.errarrs.concat(item + "</br>");
                err_num++;
            ...
            if (err_num != 0) {
              this.$alert(`${this.errarrs}url格式错误`, "提交信息有误", {
        ...
        if (this.textarea2 == "") {
          ...
          if (urllist.length > 10) {
            ...
            urllist.forEach((item, index) => {
              if (resyzm.test(item) === false) {
                this.errarrs2 = this.errarrs2.concat(item + "</br>");
                err_num++;
              } else if (this.getBLen(item) > 1024) {
                this.errarrs2 = this.errarrs2.concat(item + "</br>");
                err_num++;
            ...
            if (err_num != 0) {
              this.$alert(`${this.errarrs2}url格式错误`, "提交信息有误", {
                dangerouslyUseHTMLString: true
              });
              return false;
            } else {
