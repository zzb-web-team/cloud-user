        ...
        if (valid) {
          ...
          login(loginParams).then(data => {
            ...
            if (data.status !== 0) {
              ...
              sessionStorage.setItem("user", JSON.stringify(data.msg.username));
                sessionStorage.setItem("token", JSON.stringify(data.token));
              this.$cookies.set("user", data.msg.username, 7 * 24 * 60 * 60);
              this.$cookies.set("id", data.msg.id, 7 * 24 * 60 * 60);
              this.$cookies.set("token", data.token, 7 * 24 * 60 * 60);
              this.$router.push({
                path: "/overview"
              });
            }
